...
def get_grad(X, Y, w, badge):
    n = Y.shape[0]
    indexs = np.random.choice(n, size = badge, replace=False)
    X_badge = X[indexs]
    Y_badge = Y[indexs]
    predicted_Y = predict_Y(X_badge, w)
    return -2 * np.dot(X_badge.T, (Y_badge - predicted_Y))


def predict(X, Y, w, eta, eps, badge, max_epochs):
    n = Y.shape[0]
    
    for t in range(max_epochs):
        grad = get_grad(X, Y, w, badge)
...
if __name__ == "__main__":
    ...
    Y = data.Y
    w = np.zeros(dim+1)
    w = predict(X, Y, w, 0.03, 1e-4, badge, 50000)
    print(w)